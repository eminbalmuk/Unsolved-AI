...
export async function AppShell({ children }: { children: React.ReactNode }) {
  const [locale, dictionary, theme] = await Promise.all([
    getLocale(),
    getDictionary(),
    getTheme(),
  ]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b bg-background/82 backdrop-blur-xl">
        <div className="flex h-24 w-full items-center gap-3 px-3 sm:px-4 lg:px-5">
          <Link href="/" className="-ml-1 flex shrink-0 items-center gap-4">
            <BrandLogo />
            <span>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <ThemeSwitcher theme={theme} dictionary={dictionary.common} />
